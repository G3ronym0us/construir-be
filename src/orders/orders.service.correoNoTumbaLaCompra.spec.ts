...
describe('OrdersService.createOrder — un correo fallido no tumba la compra', () => {
  ...
  /** Cada reserva atómica que `reservarInventario` llegó a ejecutar. */
  let reservas: Array<{ uuid: string; cantidad: number }>;
  let emailService: {
  ...
  const build = async () => {
    ...
    productRepo = {
      ...
      decrement: jest.fn(),
      increment: jest.fn(),
      // `reservarInventario` comprueba y descuenta en una sola instrucción.
      // El doble anota qué se reservó y responde que sí había existencias.
      createQueryBuilder: jest.fn(() => {
        const qb: Record<string, jest.Mock> = {};
        for (const m of ['update', 'set']) {
          qb[m] = jest.fn(() => qb);
        }
        qb.where = jest.fn(
          (_condicion: string, params: { uuid: string; cantidad: number }) => {
            reservas.push({ uuid: params.uuid, cantidad: params.cantidad });
            return qb;
          },
        );
  ...
  /**
   * La reserva de inventario ocurre ANTES de los correos y no se toca:
   * tragarse el fallo del correo no puede convertirse, por accidente, en
   * tragarse también el resto del método.
   *
   * La prueba miraba `decrement`, que era como se descontaba antes de que la
   * reserva pasara a ser atómica; ahora mira la reserva, que es donde hoy se
   * compromete el stock. La garantía es la misma: una vez y por la cantidad
   * pedida.
   *
   * Y se le añade la otra mitad, que es donde se cruzan los dos arreglos: para
   * cuando se mandan los correos `reservaConfirmada` ya es true, así que el
   * `finally` NO debe devolver el stock. Si lo devolviera, un SMTP caído
   * liberaría la mercancía de un pedido que sí existe y que el cliente va a
   * venir a retirar. Por eso se exige además que no haya ningún `increment`.
   */
  it('con el correo caído el inventario sigue reservado una sola vez, y no se devuelve', async () => {
    emailService.sendOrderConfirmation.mockRejectedValue(new Error('caído'));
    emailService.sendAdminNewOrder.mockRejectedValue(new Error('caído'));

    await service.createOrder(dto, null);

    expect(reservas).toEqual([{ uuid: product.uuid, cantidad: 2 }]);
